...
def test_reassembly_out_of_order_and_retransmit():
    ...
    _shot(asm, "1.1.1.1", "2.2.2.2", 5555, 143, seq + 28, 7001, b"", 0x11, 2.01)
    streams = asm.emit()
...
def test_reassembly_gap_keeps_ordering():
    ...
    seq = isn + 1
    # contiguous run [1000,1014): ab cd e f g
    _shot(asm, "1.1.1.1", "2.2.2.2", 5555, 143, seq + 4, 7000, b"e", 0x18, 1.0)
    _shot(asm, "1.1.1.1", "2.2.2.2", 5555, 143, seq, 7000, b"ab", 0x18, 1.01)
    _shot(asm, "1.1.1.1", "2.2.2.2", 5555, 143, seq + 6, 7000, b"g", 0x18, 1.02)
    _shot(asm, "1.1.1.1", "2.2.2.2", 5555, 143, seq + 2, 7000, b"cd", 0x18, 1.03)
    _shot(asm, "1.1.1.1", "2.2.2.2", 5555, 143, seq + 5, 7000, b"f", 0x18, 1.04)
